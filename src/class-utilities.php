 ...
 *
 * - \Jeherve\Color_Posts\Utilities\get_post_image( $post_id )    -- Get an image from a post.
 * - \Jeherve\Color_Posts\Utilities\calculate_colors( $post_id )  -- Calculate Average and Contrast color.
 * - \Jeherve\Color_Posts\Utilities\get_average_color( $post_id ) -- Get the average color for a specific post.
 * - \Jeherve\Color_Posts\Utilities\get_contrast( $color )        -- Get a contrast color from an average color.
 *
...
class Utilities {
	 ...
	 *
	 * @uses Images::get_image( $post_id ) to get the source of an image in a post, apply_filters()
	 *
	 * @since 1.0
	 *
	...
	public static function get_post_image( $post_id ) {
		...
		$the_image = Images::get_image( $post_id );
		...
		/**
		 * Filters the image source used to calculate the post color.
		 *
		 * @since 1.5
		 *
		 * @param string $the_image The image URL.
		 */
	 ...
	 */
	public static function calculate_colors( $post_id ) {
		$post_image = self::get_post_image( $post_id );
	 ...
	 */
	public static function get_average_color( $post_id ) {
	 ...
	 */
	public static function post_meta_flusher( $post_id ) {
		$colors = get_post_meta( $post_id, '_post_colors', true );
	 ...
	 * @since 1.5.1
	 *
	 * @param string $color HEX color code, with or without the hashtag.
	 */
